...
# ---------------- ENV ----------------
load_dotenv()
AssistantVoice = os.getenv("AssistantVoice") or "en-US-JennyNeural"
...
# ---------------- CORE TTS ----------------
def TTS(Text, func=lambda r=None: True):
    file_path = data_path(f"speech_{uuid.uuid4().hex}.mp3")
...
def TextToSpeech(Text, func=lambda r=None: True):
    ...
    responses = [
        ...
        "Please check the chat screen for remaining text.",
        "More details are available on the chat screen.",
        "Kindly see the chat screen for full response."
    ]
